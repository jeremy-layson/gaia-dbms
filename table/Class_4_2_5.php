    ...
    public function getData()
    {
        ...
        foreach ($columns as $mun => $brgys) {
            ...
            foreach ($brgys as $brgy => $col) {
                ...
                $result = $this->db->query($query = "SELECT uid,address,baranggay,trees_fb,trees_nonfb,trees_cash FROM survey WHERE is_deleted = 0 AND `address` LIKE '%" . $mun . "%' AND ($wildcard)");
                while ($row = $result->fetch_assoc()) {
