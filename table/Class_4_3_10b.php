...
/**
* 4.3-10 Monthly Expenditure of Legal PAFs
...
class Class_4_3_10b
{
    ...
    public function __construct()
    {
        ...
        $query = "SELECT * FROM `survey` WHERE is_deleted = 0 AND `type` = 'LEGAL' AND hh_head LIKE '%[322]'";
    ...
    public function getData()
    {
        ...
        foreach ($columns as $mun => $brgys) {
            ...
            foreach ($brgys as $brgy => $col) {
                ...
                $result = $this->db->query($query = "SELECT uid,address,baranggay,he_total_expenses FROM survey WHERE is_deleted = 0 AND `type` = 'LEGAL' AND `hh_head` LIKE '%[322]' AND `address` LIKE '%" . $mun . "%' AND ($wildcard)");
                while ($row = $result->fetch_assoc()) {
