...
class Class_flood_7
{
    ...
    public function getData()
    {
        ...
        $this->tbl_cols = $tbl_cols = array('100', '1K', '10K', '100K', '500K', 'none', 'noans', 'total');
        ...
        foreach ($columns as $mun => $brgys) {
            ...
            foreach ($brgys as $brgy => $col) {
                ...
                $result = $this->db->query($query = "SELECT uid,address,baranggay,flood_damage FROM survey WHERE is_deleted = 0 AND `hh_head` LIKE '%[322]' AND `address` LIKE '%" . $mun . "%' AND ($wildcard) AND (flood_5years = 'Y' OR flood_5years = 'y')");
                while ($row = $result->fetch_assoc()) {
                    
                    
                    $val = strtoupper($row['flood_damage']);
                    $val = explode("/", $val);
