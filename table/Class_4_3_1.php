...
/**
* 4.3-1 Affected Population by Gender
    ...
    public function getData()
    {
        ...
        foreach ($columns as $mun => $brgys) {
            ...
            foreach ($brgys as $brgy => $col) {
                ...
                $result = $this->db->query($query = "SELECT uid,address,baranggay,ses_total_male,ses_total_female FROM survey WHERE is_deleted = 0 AND `hh_head` LIKE '%[322]' AND `address` LIKE '%" . $mun . "%' AND ($wildcard)");
                while ($row = $result->fetch_assoc()) {
                    unset($this->unclaimed[$row['uid']]);
                    
                    foreach (array('male', 'female') as $gender) {
                        $data[$mun][$col[0]][$gender][] = $row['uid'];
                        $data[$mun][$col[0]][$gender]['COUNT'] += intval($row['ses_total_' . $gender]);
                        $data[$mun][$col[0]]['Total'][] = $row['uid'];
                        $data[$mun][$col[0]]['Total']['COUNT'] += intval($row['ses_total_' . $gender]);

                        $data[$mun]['Sub Total'][$gender][] = $row['uid'];
                        $data[$mun]['Sub Total'][$gender]['COUNT'] += intval($row['ses_total_' . $gender]);
                        $data[$mun]['Sub Total']['Total'][] = $row['uid'];
                        $data[$mun]['Sub Total']['Total']['COUNT'] += intval($row['ses_total_' . $gender]);
                        
                        $col_total['Total']['Total'][$gender][] = $row['uid'];
                        $col_total['Total']['Total'][$gender]['COUNT'] += intval($row['ses_total_' . $gender]);
                        $col_total['Total']['Total']['Total'][] = $row['uid'];
                        $col_total['Total']['Total']['Total']['COUNT'] += intval($row['ses_total_' . $gender]);
