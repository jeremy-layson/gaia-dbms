<?php  
    include('Class_4_5_5.php');
    $class = new Class_4_5_5();
    $data = $class->getData();
?>
...
<table border="1">
    ...
    <tbody>
        ...
        foreach ($data as $mun => $value) {
            ...
            echo "<td>$mun</td>";
            foreach ($class->tbl_cols as $col) {
                echo "<td><a href='/viewer.php?field=uid,address,baranggay,rpo_relocation_preferred,rpo_relocation_option,extent&id=" . implode(",", $value['Sub Total'][$col]) . "' target='_blank'>" . round($vals[$col], 1) . "</a></td>";
        ...
        echo "<tr>";
            ...
            foreach ($class->tbl_cols as $col) {
                echo "<td><a href='/viewer.php?field=uid,address,baranggay,rpo_relocation_preferred,rpo_relocation_option,extent&id=" . implode(",", $class->total[$col]) . "' target='_blank'>" . round($vals[$col], 1) . "</a></td>";
        ...
        foreach ($data as $mun => $value) {
            ...
            foreach ($value as $brgy => $pop) {
                ...
                echo "<tr data-id='$brgy'>";
                    ...
                    echo "<td>$key</td>";
                    foreach ($class->tbl_cols as $col) {
                        echo "<td><a href='/viewer.php?field=uid,address,baranggay,rpo_relocation_preferred,rpo_relocation_option,extent&id=" . implode(",", $value[$key][$col]) . "' target='_blank'>" . round($nVals[$col], 1) . "</a></td>";
...
<a target="_blank" href="/viewer.php?field=uid,address,baranggay,rpo_relocation_preferred,rpo_relocation_option,extent&id=<?=implode(',', $class->unclaimed)?>">Uncategorized Data</a>
