...
/**
* 4.3-9b - Monthly Income of Legal PAFs
...
class Class_4_3_9b
{
    ...
    public function __construct()
    {
        ...
        $query = "SELECT * FROM `survey` WHERE is_deleted = 0 AND hh_head LIKE '%[322]' AND `type` = 'LEGAL'";
    ...
    public function getData()
    {
        ...
        foreach ($columns as $mun => $brgys) {
            ...
            foreach ($brgys as $brgy => $col) {
                ...
                $result = $this->db->query($query = "SELECT uid,address,baranggay,shi_total_hh_income FROM survey WHERE is_deleted = 0 AND `type` = 'LEGAL' AND `hh_head` LIKE '%[322]' AND `address` LIKE '%" . $mun . "%' AND ($wildcard)");
                while ($row = $result->fetch_assoc()) {
