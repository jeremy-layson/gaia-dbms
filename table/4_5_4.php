<?php  
    include('Class_4_5_4.php');
    $class = new Class_4_5_4();
    $data = $class->getData();
?>
...
<table border="1">
    <h3>Table 4.5-4 Preference for Relocation Package or Cash Compensation (LGU Level)</h3>
    <thead>
        <tr>
            <td rowspan="2">Municipalities and Cities</td>
            <td colspan="2">Relocation Package</td>
            <td colspan="2">Cash Compensation</td>
...
<table border="1">
    <h3>Table 4.5-4 Preference for Relocation Package or Cash Compensation (Baranggay Level)</h3>
    <thead>
        <tr>
            <td rowspan="2">Municipalities and Cities</td>
        ...
        foreach ($data as $mun => $value) {
            ...
            foreach ($value as $brgy => $pop) {
                ...
                echo "<tr data-id='$brgy'>";
                    ...
                    echo "<td>$key</td>";
                    foreach ($class->tbl_cols as $col) {
                        echo "<td><a href='/viewer.php?field=uid,address,baranggay,rpo_relocation_option,extent&id=" . implode(",", $value[$key][$col]) . "' target='_blank'>" . round($nVals[$col], 1) . "</a></td>";
...
<a target="_blank" href="/viewer.php?field=uid,address,baranggay,rpo_relocation_option,extent&id=<?=implode(',', $class->unclaimed)?>">Uncategorized Data</a>
