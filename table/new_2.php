<?php  
    include('Class_New_2.php');
    $class = new Class_New_2();
    $data = $class->getData();
?>
...
<html>
<head>
    <title>Table ##. Relocation Experience</title>
    ...
    <tbody>
        ...
        foreach ($data as $mun => $value) {
            ...
            echo "<tr>";
                echo "<td>$mun</td>";
                foreach ($class->tbl_cols as $colm) {
                    echo "<td><a href='/viewer.php?field=uid,asset_num,address,baranggay,reloc_exp,reloc_site&id=" . implode(",", $value[$key][$colm]) . "' target='_blank'>" . round($vals[$colm], 1) . "</a></td>";
                }
        ...
        foreach ($data as $mun => $value) {
            ...
            foreach ($value as $brgy => $pop) {
                ...
                echo "<tr data-id='$brgy'>";
                    ...
                    echo "<td>$brgy</td>";
                    foreach ($class->tbl_cols as $colm) {
                        echo "<td><a href='/viewer.php?field=uid,asset_num,address,baranggay,reloc_exp,reloc_site&id=" . implode(",", $value[$brgy][$colm]) . "' target='_blank'>" . round($vals[$colm], 1) . "</a></td>";
                    }
                $percent = round( ($vals['total'] / $total_all) * 100 , 1);
                echo "<td>$percent%</td>";
                echo "</tr>";
...
<a target="_blank" href="/viewer.php?field=uid,asset_num,address,baranggay,reloc_exp,reloc_site&id=<?=implode(',', $class->unclaimed)?>">Uncategorized Data</a>
